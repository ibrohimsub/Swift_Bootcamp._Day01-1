...
let phone = "89152342343"
let maskedPhone = phone.applyPhoneMask()
print(maskedPhone) 
...
func parseZone(_ input: String) throws -> Zone {
    ...
    case 3:
        if isTriangle(input: input) {
            return Zone(shape: "triangle", phoneNumber: maskedPhone2, name: "Sovetsky district", emergencyDept: "49324", dangerLevel: "medium", coordinates: input)
        } else {
            throw MyError.myException(message: "Input not correct. Please try again!!!")
        }
    ...
    case 4:
        if isTetragon(input: input) {
            return Zone(shape: "tetragon", phoneNumber: maskedPhone2, name: "Sovetsky district", emergencyDept: "49324", dangerLevel: "high", coordinates: input)
        } else {
            throw MyError.myException(message: "Input not correct. Please try again!!!")
        }
    ...
    if args.count == 1 {
        if isIncidentInZonePattern(input: input) {
            if(isInZone(input, zone)) {
                return Incident(description: "the woman said her cat can't get off the tree", phoneNumber: maskedPhone3, type: Field.cat.rawValue, descriptionFinish: "An incident is in Sovetsky district")
                
            } else {
                return Incident(description: "the woman said her cat can't get off the tree", phoneNumber: maskedPhone3, type: Field.cat.rawValue, descriptionFinish: "An incident is not in Sovetsky district\n Switch the applicant to the common number: 88008473824")
            }
        } else {
            throw MyError.myException(message: "Input not correct. Please try again!!!")
        }
